...
3. Use a nonrepeating Daily schedule after launch. Retire the repeating 20-puzzle rotation from ranked scheduling; supply enough new content for the launch window and an ongoing buffer. Atomically reserve each puzzle for only one Daily assignment, including concurrent scheduler runs. Maintain scheduling history beyond the 30-day board window and retain known prelaunch Daily assignments so those puzzles are not recycled as new ranked puzzles. If inventory runs out, report Daily unavailable and alert the operator instead of wrapping to old content; treat the unavailable date as void for streaks. Defensively exclude repeated assignments in aggregation even if invalid schedule data slips through.
