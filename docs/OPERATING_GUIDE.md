...
Visit `/admin/`, enter the configured password, and use the catalog filters. The default list starts with today’s Daily puzzle and hides earlier issued Daily puzzles; choose **Include previous** to view that history. Previous Daily puzzles open read-only and cannot be reordered or deleted. Drag other puzzles by the handle to reorder them within a pool; drops change the live order immediately and preserve the puzzle’s published state. On touch screens, swipe an editable card right and accept its confirmation to delete it permanently. Numeric pool positions and a confirmed Delete action are also available in the editor. Every Save is live and therefore requires an emoji sequence, category, canonical accepted answer, three hints, and an explanation. ETags prevent an older tab from overwriting a newer edit or deletion.
