  ...
  return (
    <main className="admin-shell">
      ...
      <section className="feedback-list">
        {visible.map((item) => <article className="feedback-item" key={item.id}><div className={`rating-mark ${item.rating}`} aria-label={item.rating === "up" ? "Thumbs up" : "Thumbs down"}>{item.rating === "up" ? "👍" : "👎"}</div><div><div className="feedback-meta"><strong>Puzzle #{item.puzzleNumber}</strong><span>{item.puzzlePool}</span><time dateTime={item.createdAt}>{new Date(item.createdAt).toLocaleString()}</time></div><p>{item.comment || <em>No written comment</em>}</p><small>{item.outcome} · {item.guessCount} guesses · {item.hintCount} hints</small></div></article>)}
