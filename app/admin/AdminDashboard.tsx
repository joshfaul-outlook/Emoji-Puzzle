  ...
  return (
    <main className="admin-shell">
      ...
      <section className="puzzle-list" aria-label="Puzzle catalog">
        {visible.map((puzzle) => (
          <div className={`puzzle-row ${moving === puzzle.id ? "is-moving" : ""}`} key={puzzle.id} onDragOver={(event) => { if (canReorder) { event.preventDefault(); event.dataTransfer.dropEffect = "move"; } }} onDrop={(event) => { event.preventDefault(); if (draggedId) void movePuzzle(draggedId, puzzle.id); }}>
            {canReorder && <button className="drag-handle" type="button" draggable={true} aria-label={`Drag ${puzzle.answer} to reorder`} onDragStart={(event) => { event.dataTransfer.effectAllowed = "move"; event.dataTransfer.setData("text/plain", puzzle.id); setDraggedId(puzzle.id); }} onDragEnd={() => setDraggedId(null)} onKeyDown={(event) => { if (event.key !== "ArrowUp" && event.key !== "ArrowDown") return; event.preventDefault(); const siblings = puzzles.filter((item) => item.pool === puzzle.pool).sort((a, b) => a.position - b.position || a.number - b.number); const index = siblings.findIndex((item) => item.id === puzzle.id); const target = siblings[index + (event.key === "ArrowUp" ? -1 : 1)]; if (target) void movePuzzle(puzzle.id, target.id); }}>⠿</button>}
