...
import type { PlayerStats as Stats, PlaySummary, RankingsPage } from "../lib/player-stats";
...
export function PlayerStatsPanel({ identity, onClose }: { identity: PlayerIdentity; onClose: () => void }) {
  const dialog = useRef<HTMLDialogElement>(null);
  const [tab, setTab] = useState<"stats" | "rankings">("stats");
  ...
  return <dialog ref={dialog} className="stats-dialog" aria-labelledby="stats-title" onCancel={(event) => { event.preventDefault(); onClose(); }} onKeyDown={(event) => {
    ...
    <div className="stats-tabs" role="group" aria-label="Stats view">
      <button type="button" aria-pressed={tab === "stats"} onClick={() => setTab("stats")}>My stats</button>
      <button type="button" aria-pressed={tab === "rankings"} onClick={() => { setTab("rankings"); void loadBoard(); }}>Daily rankings</button>
    </div>
    {tab === "stats" ? <div aria-busy={loading}>
      ...
      {stats && !loading && !statsError && <>
        <div className="streak-cards" aria-label="Daily streaks"><div><strong>{stats.streaks.current}</strong><span>Current Daily streak</span></div><div><strong>{stats.streaks.best}</strong><span>Best Daily streak</span></div></div>
        <p className="stats-note">Consecutive Daily solves, including hints. Streaks use your full ranked history and can exceed 30 days. Each Daily closes at midnight UTC.</p>
        ...
        <Summary title="Practice challenges" summary={stats.challenges} />
        ...
        {stats.coverageStart && <p className="stats-note">Recorded history begins {dateLabel(stats.coverageStart)}. Updated {timestamp(stats.asOf)}.</p>}
      </>}
      {preferenceError && <p role="alert">{preferenceError}</p>}
    </div> : <div aria-busy={boardLoading}>
      ...
      {stats && <p className="own-rank">{!stats.publicStats ? "Your Daily stats are private. You can change this in My stats." : stats.ownRank ? `Your rank: #${stats.ownRank.rank} · ${stats.ownRank.solves} Daily solves` : stats.rankingsStatus === "ready" ? "Solve a Daily puzzle to join the rankings." : "Your rank is temporarily unavailable."}</p>}
...
export function DailyStreak({ identity }: { identity: PlayerIdentity }) {
  ...
  useEffect(() => {
    const controller = new AbortController();
    fetch("/api/players/me/stats", { headers: playerHeaders(identity), signal: controller.signal }).then(readResponse<Stats>).then((data) => {
      if (!controller.signal.aborted && data.launchDate) setStreak(data.streaks.current);
    }).catch(() => undefined);
    return () => controller.abort();
  }, [identity]);
  return streak === null ? null : <p className="daily-streak" role="status">{streak}-day Daily streak</p>;
}
